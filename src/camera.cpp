...
Ray Camera::generate_ray(double x, double y) const {
  ...
  // canonical sensor plane one unit away from the pinhole.
    ...
    double xx = (2*x-1)*tan(radians(hFov/2.0f));
    double yy = (2*y-1)*tan(radians(vFov/2.0f));
    
    //double ratio = screenW/(double)screenH;
    
    Vector3D pix_pos = Vector3D(xx,yy,-1);
    Vector3D cam_pos = Vector3D(0,0,0);
    Vector3D ray_dir = pix_pos - cam_pos;
    
    ray_dir = c2w * ray_dir;
    ray_dir.normalize();
    
    
    Ray ray = Ray(this->pos, ray_dir);
    ray.min_t = 0;
    ray.max_t = std::numeric_limits<double>::infinity();
