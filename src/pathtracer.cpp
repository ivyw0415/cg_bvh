...
Spectrum PathTracer::trace_ray(const Ray &r) {
  ...
  if (!bvh->intersect(r, &isect)) {
    ...
    // samples from the environment map. If you don't return black.
      ...
      float * pdf = 0;
      if (envLight != nullptr) {
          //return envLight->sample_L(r.o, wi, distToLight, pdf);
  ...
  //DirectionalLight light(Spectrum(5.f, 5.f, 5.f), Vector3D(1.0, -1.0, 0.0));
    ...
    for (SceneLight * light : scene->lights) {
        ...
        for (int i=0; i<num_light_samples; i++) {
            ...
            Vector3D ori = hit_p + EPS_D * dir_to_light;
      
            Ray s_ray = Ray(ori, dir_to_light,dist_to_light,0);
            
            Intersection intersect;
            if (!bvh->intersect(s_ray, &intersect)){
                L_out +=  f * light_L * (cos_theta / pdf) * scale;
            }
        }
    }
  // TODO:
  ...
  // traveling down one path forever.
    ...
    if (r.depth<=max_ray_depth) {
        Vector3D wi;
        float pdf_i;
        Spectrum f_i = isect.bsdf->sample_f(w_out, &wi, &pdf_i);
       
        wi.normalize();
        Vector3D w_wi = o2w * wi;
        w_wi.normalize();
        Vector3D i_ori = hit_p + EPS_D * w_wi;
        //cout<<w_wi.z<<endl;
        double terminateProbability = 1.f - f_i.illum();
        double rand = (double)std::rand()/RAND_MAX;
                
        if (rand < terminateProbability) {

        } else {
           L_out += (f_i * trace_ray(Ray(i_ori, w_wi, INF_D, r.depth+1))) * (1/(pdf_i * (1-terminateProbability)));
        }        
    }
    return L_out;
}
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  int num_samples = ns_aa;
    ...
    //double ratio = (double)frameBuffer.w/(double)frameBuffer.h;
   
    Spectrum spectrum_result; // = trace_ray(camera->generate_ray((double)x/frameBuffer.w, (double)y/frameBuffer.h));
    ...
    for (int i=0; i<num_samples; i++) {
        ...
        double p_x = (x + p.x)/(double)sampleBuffer.w;
        double p_y = (y + p.y)/(double)sampleBuffer.h;
        Vector2D p_r = Vector2D(p_x,p_y);
        spectrum_result += trace_ray(camera->generate_ray(p_r.x, p_r.y));
        //}
    }
    
    spectrum_result.r = spectrum_result.r / (double)(num_samples);
    spectrum_result.g = spectrum_result.g / (double)(num_samples);
    spectrum_result.b = spectrum_result.b / (double)(num_samples);
