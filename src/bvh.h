...
struct BVHNode {
  ...
  BVHNode* r;     ///< right child node
    ...
    struct ClosestHit {
        ClosestHit(BVHNode* root, double min_t){}
        BVHNode* root;
        double min_t;
    };
...
class BVHAccel : public Aggregate {
 public:
  ...
  bool intersect(const Ray& r, Intersection* i) const;
    ...
    bool intersect(const Ray& r, Intersection* i, BVHNode * root) const;
    bool intersect(const Ray& r, BVHNode * root) const;
  /**
  ...
  void drawOutline(const Color& c) const { }
    ...
    void construct(BVHNode* root, size_t start, size_t range, size_t max_leaf_size);

 private:
  BVHNode* root; ///< root node of the BVH
    //ClosestHitInfo closet;
};
